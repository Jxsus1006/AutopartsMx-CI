...
if ($id_transaccion == '') {
  ...
  if ($sql->fetchColumn() > 0) {
    ...
    $sql = $con->prepare("SELECT id_compra, fecha, correo, total FROM compra WHERE id_transaccion=? 
        AND status=? LIMIT 1");
    $sql->execute([$id_transaccion, 'COMPLETED']);
    ...
    $sqlDet = $con->prepare("SELECT nombre_pieza, precio, cantidad FROM detalle_compra WHERE id_compra=?");
              ...
              <tbody>
                <?php while ($row_det = $sqlDet->fetch(PDO::FETCH_ASSOC)) {
                  ...
                  <tr>
                    ...
                    <td><?php echo $row_det['nombre_pieza']; ?></td>
